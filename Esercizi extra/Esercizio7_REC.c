/*	Si crei una funzione RICORSIVA chiamata PicchiREC che restituisca il numero di picchi che la lista contiene.
...
int PicchiREC(ElementoDiLista *lista);
...
int PicchiREC(ElementoDiLista *lista){
	...
	if(((float)(lista->info) < (float)(lista->next->info)/2) && ((float)(lista->next->next->info) < (float)(lista->next->info)/2))
		return 1+PicchiREC(lista->next);
	else return PicchiREC(lista->next);
